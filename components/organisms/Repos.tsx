...
const Repos: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRepos = async () => {
            const response = await fetch('https://gh-pinned-repos.egoist.dev/?username=hemantwasthere');
                ...
                {repos.length > 0 ? (
                    <>
                        {repos.map(({ link, owner, repo, description, languageColor, language, stars, forks }) => (
                            <Link
                                className='no-underline text-white h-full rounded-[8px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow'
                                href={link}
                                key={link}
                                target="_blank"
                                rel="noreferrer"
